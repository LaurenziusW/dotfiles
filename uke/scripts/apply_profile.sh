...
# Form factor: ${UKE_FORM_FACTOR}
# Computed font size: ${font_size}
EOF
...
main() {
    ...
    if [[ "${DRY_RUN}" != "--dry-run" ]]; then
        if [[ "${UKE_OS}" == "arch" ]] || [[ "${UKE_OS}" == "linux" ]]; then
            ...
            info "Reload yabai: yabai --restart-service"
        fi
    fi
}
